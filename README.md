# 👋 Hi! I'm Muhamad Juwandi
### Data Science Student | Junior Graphic Designer | 日本のAI企業を目指すデータサイエンティスト志望者

📍 Indonesia | 💼 Freelance at Naeta | 🎓 Universitas Terbuka  
🧠 Currently Learning: Machine Learning, NLP, and Japanese Language  
🌏 Target: Data Science Internship (Remote) — Japan & International

---

### 🧰 Tech Stack
Python · Javascript · Pandas · NumPy · Scikit-learn · Matplotlib · SQL · MongoDB  
Git · GitHub · WordPress · Canva · Adobe
 

---

### 📊 GitHub Statistics
<p align="left">
<a href="https://github.com/MuhamadJuwandi">
  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=MuhamadJuwandi&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true"/>
  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=MuhamadJuwandi&layout=compact&langs_count=7&theme=tokyonight"/>
</a>
</p>

---

### 🌸 自己紹介 (Japanese)
こんにちは、ジュワンディです。現在インドネシアでデータサイエンスを専攻しており、  
将来的には日本企業でAIやデータ分析に携わることを目指しています。  
GitHubでは学習プロジェクトや分析結果を公開しています。

---

### 📬 Contact
- [LinkedIn]([www.linkedin.com/in/muhamadjuwandi])
- [Portfolio Website]([https://keunal.id])
- [Email]([mailto:muhamadjuwandi28@gmail.com])
